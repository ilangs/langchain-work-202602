...
# 4. 이미지 파일 로드
image = Image.open("./images/cat.jpg").convert("RGB") # RGB로 변환

# 5. 이미지 -> 벡터 변환
vector = model.encode([image]) # 반드시 리스트 형태로 넣어서 (1,512) 배치 형태로 생성 => 정수 반환

# FAISS는 float32 타입만 허용
vector = vector.astype("float32") # 실수 형태로 반환 (검색 정확도 향상) 예) 5 => 5.767 
...
text_vector = model.encode([query_text]) # 반드시 리스트 형태로 입력, 텍스트 -> 벡터(숫자) => 정수 반환
text_vector = text_vector.astype("float32") # 실수 형태로 생성

# 8. 유사도 벡터 검색
distances, indices = index.search(text_vector, k=1) # k=1 => 상위 1개의 유사 검색 결과를 반환

# 저장 확인 출력
print(f"거리값: {distances}") # 거리값(유사도) 그리고 인덱스(위치)를 출력
