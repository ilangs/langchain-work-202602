...
def cook_node(state: ChefState):
    ...
    return {
        ...
        "errors":[]    # 수정: 정상 처리 가능하도록 빈 리스트 반환
    }
...
#########################################################################
workflow.add_node("marketing", marketing_node) # [홍보부] 메뉴 홍보 부서
#########################################################################
...
if st.button("🚀 전 부서 협업 시스템 가동"):
    ...
    #5. 사용자가 로드맵을 소장할 수 있게 다운로드 버튼을 만듭니다.
    st.download_button(
        label="📂 협업 로드맵 저장하기", 
        data=path_img_data, 
